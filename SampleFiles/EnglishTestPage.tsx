...
import { Globe, Send, AlertCircle, ArrowLeft } from 'lucide-react';

interface EnglishTestPageProps {
  onBack?: () => void;
  onSubmit?: (testResponse: string) => void;
}

const EnglishTestPage = ({ onBack, onSubmit }: EnglishTestPageProps) => {
  const [testResponse, setTestResponse] = useState('');
  const [submitted, setSubmitted] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState('');

  // Sample news images - in production, these would be dynamic
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      setSubmitted(true);
      // Call onSubmit after a brief delay to show the success message
      setTimeout(() => {
        if (onSubmit) {
          onSubmit(testResponse);
        }
      }, 3000);
    }, 1500);
  };
